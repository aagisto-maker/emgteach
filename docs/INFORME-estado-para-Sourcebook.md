...
- **`src/emgteach/gui/help_texts.py:85`**
  - EN: After 10 s of warm-up, 3 brief maximal efforts of 1.5 s are recorded for each muscle, each announced 3 s ahead and followed by 2 s of rest. The reference is the strongest 0.2 s across the repetitions kept, so it is a maximum the task cannot exceed; a repetition that came out weak can be discarded afterwards in the analysis.
  - ES: Tras 10 s de calentamiento se graban 3 esfuerzos máximos breves de 1,5 s por músculo, cada uno anunciado con 3 s de antelación y seguido de 2 s de descanso. La referencia es el tramo de 0,2 s más fuerte de las repeticiones que se conservan, de modo que es un máximo que la tarea no puede superar; una repetición que salió floja puede descartarse después en el análisis.
