...
_ES = {
    ...
    'Manoeuvre': 'Maniobra',
    'The app suggests the informative fragments (active periods), one per contraction. Adjust, add or remove them; only the checked fragments are analysed. You decide the final selection.': 'La aplicación propone los fragmentos informativos (periodos activos), uno por contracción. Ajústelos, añada o quite; solo se analizan los marcados. La selección final la decide usted.',
    'Name a fragment with the manoeuvre, not the muscle: in «Flexion» both channels are recorded, the flexor as agonist and the extensor as antagonist, and that pair is what the co-activation table measures. Give consecutive fragments the same name and they become one window of that table — six efforts named «Flexion» are six samples of one condition, not six conditions. Nothing can name them for you: the detector finds where a contraction started, not which manoeuvre it was. That is read off the trace, and reading it is the exercise.': 'Nombre el fragmento con la maniobra, no con el músculo: en «Flexión» se registran los dos canales, el flexor como agonista y el extensor como antagonista, y ese par es lo que mide la tabla de coactivación. Si se da el mismo nombre a fragmentos consecutivos pasan a ser una sola ventana de esa tabla: seis esfuerzos llamados «Flexión» son seis muestras de una condición, no seis condiciones. Nadie puede nombrarlos por usted: el detector encuentra dónde empezó una contracción, no qué maniobra era. Eso se lee en la traza, y leerlo es el ejercicio.',
