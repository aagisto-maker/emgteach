...
class MvcTab(QWidget):
    ...
    def _build_entry_panel(self) -> QWidget:
        ...
        for paragraph in (
            ...
            tr(
                "A reference is only a yardstick if it recruits the same muscle "
                "mass as the task, and a surface electrode does not see one "
                "muscle but the compartment beneath it. So the reference is a "
                "brief, explosive maximal jerk of the movement the muscle "
                "makes, not a sustained push against something fixed: a push "
                "has to be braced, which switches on the antagonist as well. "
                "On the forearm — the pair of the practical guide — the "
                "flexor's is a jerk of wrist flexion with the fist clenched "
                "with all one's strength, and the extensor's a jerk of wrist "
                "extension with the hand open and the fingers stretched out as "
                "far as they go: the task includes a grip, and clenching the "
                "fist brings in the finger flexors of the same compartment, "
                "which a push of the wrist leaves out. With another pair — "
                "biceps and triceps, say — the gesture is that pair's own, "
